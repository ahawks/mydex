// Type: Dexcom.ReceiverApi.ManufacturingParameterRecord
...
{
  public class ManufacturingParameterRecord : IGenericRecord
  {
    private ReceiverManufacturingParameterRecord m_internalRecord;
    private XManufacturingParameterRecord m_xmlRecord;

    public int RecordRevision { get; private set; }

    public XManufacturingParameterRecord Parameters
    {
      get
      {
    ...
    public ManufacturingParameterRecord(byte[] data, int offset)
      : this(data, offset, ReceiverTools.GetLatestSupportedRecordRevision(ReceiverRecordType.ManufacturingData))
    {
    }

    public ManufacturingParameterRecord(byte[] data, int offset, int recordRevision)
    {
      this.m_internalRecord = (ReceiverManufacturingParameterRecord) ReceiverTools.DatabaseRecordFactory(this.m_internalRecord.RecordType, recordRevision, data, offset);
    ...
    public ManufacturingParameterRecord()
    {
      this.m_xmlRecord = new XManufacturingParameterRecord();
    }

    private XManufacturingParameterRecord DoCreateXmlRecord(string xmlData)
    {
      XmlDocument xmlDocument = new XmlDocument();
      xmlDocument.LoadXml(xmlData);
      return new XManufacturingParameterRecord(xmlDocument.SelectSingleNode("ManufacturingParameters") as XmlElement);
    }
