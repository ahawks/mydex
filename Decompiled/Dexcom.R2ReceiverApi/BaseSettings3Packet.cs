// Type: Dexcom.R2Receiver.BaseSettings3Packet
...
namespace Dexcom.R2Receiver
{
  ...
  public struct BaseSettings3Packet
  {
    public static readonly int NumBytes = 6 + Marshal.SizeOf(typeof (R2Settings3Record));
    private byte m_SOF;
    private R2Commands m_command;
    private short m_size;
    private R2Settings3Record m_settings;
    ...
    public static int PacketSize
    {
      get
      {
        return BaseSettings3Packet.NumBytes;
      }
    }
    ...
    static BaseSettings3Packet()
    {
    }

    public BaseSettings3Packet(R2Commands command, R2Settings3Record settings)
    {
      this.m_SOF = (byte) 1;
      this.m_command = command;
      this.m_size = Convert.ToInt16(BaseSettings3Packet.NumBytes - 6);
      this.m_settings = settings;
      this.m_crc = (ushort) 0;
      byte[] bytes = this.Bytes;
      this.m_crc = Crc.CalculateCrc16(bytes, 0, BaseSettings3Packet.NumBytes - 2);
    ...
    public BaseSettings3Packet(byte[] bytes)
    {
      R2Commands r2Commands = R2Commands.ACK;
      if (bytes.Length < BaseSettings3Packet.PacketSize)
        throw new ApplicationException("Not enough bytes in argument used to create packet.");
      this.m_SOF = (byte) 1;
      this.m_command = R2Commands.Unknown;
      this.m_size = (short) 0;
      this.m_settings = new R2Settings3Record();
      this.m_crc = (ushort) 0;
      this = (BaseSettings3Packet) DataTools.ConvertBytesToObject(bytes, 0, this.GetType());
      if ((int) this.m_SOF != 1)
      ...
      if ((int) this.m_size != BaseSettings3Packet.NumBytes - 6)
        throw new ApplicationException("Bad Packet. Size field not valid!");
      if ((int) this.m_crc != (int) Crc.CalculateCrc16(bytes, 0, BaseSettings3Packet.NumBytes - 2))
