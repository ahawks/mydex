// Type: Dexcom.R2Receiver.Base4BytePacket
...
namespace Dexcom.R2Receiver
{
  ...
  public struct Base4BytePacket
  {
    public const int NumBytes = 10;
    private byte m_SOF;
    ...
    public Base4BytePacket(R2Commands command, uint payload)
    {
      this.m_SOF = (byte) 1;
      ...
      this.m_crc = (ushort) 0;
      this.m_crc = Crc.CalculateCrc16(this.Bytes, 0, 8);
    }

    public Base4BytePacket(R2Commands command, byte[] bytes)
    {
      if (bytes.Length < Base4BytePacket.PacketSize)
      ...
      this.m_payload = 0U;
      this.m_crc = (ushort) 0;
      this = (Base4BytePacket) DataTools.ConvertBytesToObject(bytes, 0, this.GetType());
      if ((int) this.m_SOF != 1)
      ...
      if ((int) this.m_size != 4)
        throw new ApplicationException("Bad Packet. Size field not valid!");
      if ((int) this.m_crc != (int) Crc.CalculateCrc16(bytes, 0, 8))
