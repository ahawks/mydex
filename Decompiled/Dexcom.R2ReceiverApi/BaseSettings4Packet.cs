// Type: Dexcom.R2Receiver.BaseSettings4Packet
...
namespace Dexcom.R2Receiver
{
  ...
  public struct BaseSettings4Packet
  {
    public static readonly int NumBytes = 6 + Marshal.SizeOf(typeof (R2Settings4Record));
    private byte m_SOF;
    private R2Commands m_command;
    private short m_size;
    private R2Settings4Record m_settings;
    ...
    public static int PacketSize
    {
      get
      {
        return BaseSettings4Packet.NumBytes;
      }
    }
    ...
    static BaseSettings4Packet()
    {
    }

    public BaseSettings4Packet(R2Commands command, R2Settings4Record settings)
    {
      this.m_SOF = (byte) 1;
      this.m_command = command;
      this.m_size = Convert.ToInt16(BaseSettings4Packet.NumBytes - 6);
      this.m_settings = settings;
      this.m_crc = (ushort) 0;
      byte[] bytes = this.Bytes;
      this.m_crc = Crc.CalculateCrc16(bytes, 0, BaseSettings4Packet.NumBytes - 2);
    ...
    public BaseSettings4Packet(byte[] bytes)
    {
      R2Commands r2Commands = R2Commands.ACK;
      if (bytes.Length < BaseSettings4Packet.PacketSize)
        throw new ApplicationException("Not enough bytes in argument used to create packet.");
      this.m_SOF = (byte) 1;
      this.m_command = R2Commands.Unknown;
      this.m_size = (short) 0;
      this.m_settings = new R2Settings4Record();
      this.m_crc = (ushort) 0;
      this = (BaseSettings4Packet) DataTools.ConvertBytesToObject(bytes, 0, this.GetType());
      if ((int) this.m_SOF != 1)
      ...
      if ((int) this.m_size != BaseSettings4Packet.NumBytes - 6)
        throw new ApplicationException("Bad Packet. Size field not valid!");
      if ((int) this.m_crc != (int) Crc.CalculateCrc16(bytes, 0, BaseSettings4Packet.NumBytes - 2))
